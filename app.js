...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentScenarioKey = 'mildlySedated';
...
// Innsiktspanel
    ...
    const SCENARIOS = {
        ...
        mildlySedated: {
            ...
            annotations: [
                { track: 'paw', relX: 0.42, relY: 0.32, title: 'Myk forkant (200 ms)', desc: 'Stigetid 200 ms gir en myk og skånsom trykkstigning uten oversving' },
                { track: 'flow', relX: 0.58, relY: 0.55, title: 'Fullstendig tømming', desc: 'Flow returnerer helt til null før neste pust (ingen auto-PEEP)' },
                { track: 'vol', relX: 0.45, relY: 0.30, title: 'Fysiologisk Vt', desc: 'ΔP 5 cmH₂O gir adekvat tidalvolum (~545 ml, 7,7 ml/kg PBW)' }
            ]
        },
        ...
        slowTrigger: {
            ...
            compliance: 24, resistance: 6, expRatio: 1.2, flowLimitation: 0.0,
        ...
        autotrigger: {
            ...
            riseTime: 150, cycling: 25, tiSet: 1.0, tiMax: 2.0, leak: 55,
        ...
        earlyCycle: {
            ...
            rrSpont: 28, pmus: 13, tiNeural: 1.00, pmusExp: 0.0,
        ...
        lowDrive: {
            ...
            rrSpont: 5, pmus: 3.0, tiNeural: 0.9, pmusExp: 0.0,
    ...
    function setFlowLimitationSeverity(level) {
        ...
        if (sliders.criticalClosingPressure) sliders.criticalClosingPressure.disabled = !isCustom;
        if (sliders.flowConductance) sliders.flowConductance.disabled = !isCustom;
        if (sliders.peepStenting) sliders.peepStenting.disabled = !isCustom;
        document.querySelectorAll('.step-btn[data-target="sliderCriticalClosingPressure"], .step-btn[data-target="sliderFlowConductance"], .step-btn[data-target="sliderPeepStenting"]').forEach(btn => btn.disabled = !isCustom);
    ...
    function updateSimulatorFromUI() {
        ...
        // 5.1: Lås modulen aktiv når rrSpont > 0, deaktiver ved passiv pasient
        const cardRecoil = document.getElementById('cardRecoil');
        if (cardRecoil) {
            if (rrSpont <= 0) {
                cardRecoil.classList.add('control-disabled');
                if (sliders.recoil) sliders.recoil.disabled = true;
            } else {
        ...
        if (cardResponsivePmus) {
            if (rrSpont <= 0) {
                cardResponsivePmus.classList.add('control-disabled');
                if (checkResponsivePmus) checkResponsivePmus.disabled = true;
                if (sliders.responsiveness) sliders.responsiveness.disabled = true;
                document.querySelectorAll('.step-btn[data-target="sliderResponsiveness"]').forEach(btn => btn.disabled = true);
            } else {
        ...
        if (cardPmusOffset) {
            if (rrSpont <= 0) {
                cardPmusOffset.classList.add('control-disabled');
                if (sliders.pmusOffset) sliders.pmusOffset.disabled = true;
                document.querySelectorAll('.step-btn[data-target="sliderPmusOffset"]').forEach(btn => btn.disabled = true);
            } else {
    ...
    Object.values(sliders).forEach(slider => {
        ...
        slider.addEventListener('input', () => {
            if (slider === sliders.compliance || slider === sliders.resistance ||
                ...
                slider === sliders.triseNeural || slider === sliders.tholdNeural ||
                slider === sliders.tdecayNeural) {
    ...
    function applyPreset(presetKey) {
        ...
        if (presetKey === 'copd') {
            setSeverityButtonActive('moderate');
            if (sliders.criticalClosingPressure) {
                sliders.criticalClosingPressure.value = '8.0';
                sliders.criticalClosingPressure.disabled = true;
            }
            if (sliders.flowConductance) {
                sliders.flowConductance.value = '0.50';
                sliders.flowConductance.disabled = true;
            }
            if (sliders.peepStenting) {
                sliders.peepStenting.value = '70';
                sliders.peepStenting.disabled = true;
            }
            document.querySelectorAll('.step-btn[data-target="sliderCriticalClosingPressure"], .step-btn[data-target="sliderFlowConductance"], .step-btn[data-target="sliderPeepStenting"]').forEach(btn => btn.disabled = true);
        } else {
            setSeverityButtonActive('none');
            if (sliders.criticalClosingPressure) {
                sliders.criticalClosingPressure.value = '0.0';
                sliders.criticalClosingPressure.disabled = true;
            }
            if (sliders.flowConductance) {
                sliders.flowConductance.value = '1.00';
                sliders.flowConductance.disabled = true;
            }
            if (sliders.peepStenting) {
                sliders.peepStenting.value = '0';
                sliders.peepStenting.disabled = true;
            }
            document.querySelectorAll('.step-btn[data-target="sliderCriticalClosingPressure"], .step-btn[data-target="sliderFlowConductance"], .step-btn[data-target="sliderPeepStenting"]').forEach(btn => btn.disabled = true);
        }
        if (sliders.recoil) sliders.recoil.value = simulator.patient.recoilStrength !== undefined ? simulator.patient.recoilStrength : 25;
        ...
        if (sliders.pmusOffset) sliders.pmusOffset.value = '0.00';
        ...
        if (sliders.triseNeural) sliders.triseNeural.value = (0.30 * simulator.patientDrive.tiNeural).toFixed(2);
        if (sliders.tholdNeural) sliders.tholdNeural.value = '0.00';
        if (sliders.tdecayNeural) sliders.tdecayNeural.value = (0.40 * simulator.patientDrive.tiNeural).toFixed(2);
    ...
    function applyScenario(scenarioKey) {
        ...
        if (scen.criticalClosingPressure !== undefined) {
            setSeverityButtonActive('custom');
            if (sliders.criticalClosingPressure) {
                sliders.criticalClosingPressure.value = scen.criticalClosingPressure;
                sliders.criticalClosingPressure.disabled = false;
            }
            if (sliders.flowConductance) {
                sliders.flowConductance.value = scen.flowConductance !== undefined ? scen.flowConductance : 1.0;
                sliders.flowConductance.disabled = false;
            }
            if (sliders.peepStenting) {
                sliders.peepStenting.value = scen.peepStenting !== undefined ? scen.peepStenting : 0;
                sliders.peepStenting.disabled = false;
            }
            ...
            const fl = scen.flowLimitation !== undefined ? scen.flowLimitation : 0.0;
            if (fl === 0) setSeverityButtonActive('none');
            else if (Math.abs(fl - 0.35) < 0.05) setSeverityButtonActive('mild');
            else if (Math.abs(fl - 0.70) < 0.05 || Math.abs(fl - 0.60) < 0.05) setSeverityButtonActive('moderate');
            else if (Math.abs(fl - 1.00) < 0.05) setSeverityButtonActive('severe');
            else setSeverityButtonActive('custom');

            const pCrit = (fl / 0.70) * 8.0;
            const gFl = Math.max(0.1, 1.0 - (fl / 0.70) * 0.5);
            const stent = fl > 0 ? 70 : 0;
            if (sliders.criticalClosingPressure) {
                sliders.criticalClosingPressure.value = pCrit.toFixed(1);
                sliders.criticalClosingPressure.disabled = true;
            }
            if (sliders.flowConductance) {
                sliders.flowConductance.value = gFl.toFixed(2);
                sliders.flowConductance.disabled = true;
            }
            if (sliders.peepStenting) {
                sliders.peepStenting.value = stent;
                sliders.peepStenting.disabled = true;
            }
            document.querySelectorAll('.step-btn[data-target="sliderCriticalClosingPressure"], .step-btn[data-target="sliderFlowConductance"], .step-btn[data-target="sliderPeepStenting"]').forEach(btn => btn.disabled = true);
        }
        if (sliders.recoil) sliders.recoil.value = scen.recoil !== undefined ? scen.recoil : (scen.rrSpont > 0 ? 25 : 0);
        ...
        if (checkResponsivePmus) checkResponsivePmus.checked = !!scen.responsivePmus;
        if (sliders.responsiveness) sliders.responsiveness.value = scen.responsiveness !== undefined ? scen.responsiveness : 50;
        if (sliders.tiNeural) sliders.tiNeural.value = scen.tiNeural;
        if (checkKobleTiNeural) {
            if (scen.kobleTiNeural !== undefined) {
                checkKobleTiNeural.checked = scen.kobleTiNeural;
            } else if (scen.triseNeural !== undefined || scen.tholdNeural !== undefined || scen.tdecayNeural !== undefined) {
                checkKobleTiNeural.checked = false;
            } else {
                checkKobleTiNeural.checked = true;
            }
        }
        if (sliders.triseNeural) sliders.triseNeural.value = scen.triseNeural !== undefined ? scen.triseNeural : (0.30 * scen.tiNeural).toFixed(2);
        if (sliders.tholdNeural) sliders.tholdNeural.value = scen.tholdNeural !== undefined ? scen.tholdNeural : '0.00';
        if (sliders.tdecayNeural) sliders.tdecayNeural.value = scen.tdecayNeural !== undefined ? scen.tdecayNeural : (0.40 * scen.tiNeural).toFixed(2);
    ...
    Object.entries(scenarioBtns).forEach(([key, btn]) => {
        if (btn) {
            btn.addEventListener('click', () => {
                if (btnTestHighSupport) btnTestHighSupport.classList.remove('active');
                applyScenario(key);
            });
        }
    });

    if (btnTestHighSupport) {
        btnTestHighSupport.addEventListener('click', () => {
            if (sliders.ipap) {
                sliders.ipap.value = 16;
            }
            btnTestHighSupport.classList.add('active');
            if (scenarioBtns.mildlySedated) scenarioBtns.mildlySedated.classList.remove('active');
            isScenarioActive = false;
            renderer.setAnnotations([]);
    ...
    if (btnToggleAnnotations) {
        btnToggleAnnotations.addEventListener('click', () => {
            if (renderer.annotations.length === 0 && SCENARIOS.mildlySedated && SCENARIOS.mildlySedated.annotations) {
                renderer.setAnnotations(SCENARIOS.mildlySedated.annotations);
            }
            const isShown = renderer.toggleAnnotations();
            setAnnotationButtonState(isShown);
            if (isShown) {
                showToast('💡 <strong>Fasit aktivert:</strong> Viser pedagogiske piler og markeringer for scenariet.');
            }
        });
    }
    ...
    btnReset.addEventListener('click', () => {
        ...
        applyScenario('mildlySedated');
        showToast('↺ Simulering nullstilt til utgangspunkt (Lungefrisk, lett sedert).');
    ...
    function updateInsights() {
        ...
        if (insightText && !isScenarioActive) {
            const currentDeltaP = Math.max(0, simulator.settings.ipap - simulator.settings.epap);
            const currentVt = simulator.state.measured.vt || insights.theoreticalVt;
            const vtPerKg = (currentVt / 71).toFixed(1);
            let evalHtml = '';

            if (currentDeltaP <= 3) {
                evalHtml = `
                    <div style="margin-bottom: 8px; padding: 8px 12px; background: rgba(234, 179, 8, 0.15); border-left: 3px solid #eab308; border-radius: 4px; font-size: 13px; line-height: 1.45; color: #fef08a;">
                        ⚡ <strong>Svært lav trykkstøtte (ΔP ${currentDeltaP} cmH₂O):</strong> Tidalvolumet er i underkant (~${currentVt} ml / ${vtPerKg} ml/kg). Selv med normale lunger trenger en lett sedert pasient litt støtte (ca. 4–6 cmH₂O) for å opprettholde adekvat alveolær ventilasjon.
                    </div>`;
            } else if (currentDeltaP <= 7) {
                evalHtml = `
                    <div style="margin-bottom: 8px; padding: 8px 12px; background: rgba(34, 197, 94, 0.15); border-left: 3px solid #22c55e; border-radius: 4px; font-size: 13px; line-height: 1.45; color: #86efac;">
                        ✅ <strong>Optimal trykkstøtte (ΔP ${currentDeltaP} cmH₂O):</strong> Perfekt tilpasset normale lunger! Tidalvolumet er fysiologisk og lungeprotektivt (~${currentVt} ml / ${vtPerKg} ml/kg PBW). Pasienten har et rolig og uanstrengt pustemønster uten risiko for overventilering.
                    </div>`;
            } else if (currentDeltaP <= 10) {
                evalHtml = `
                    <div style="margin-bottom: 8px; padding: 8px 12px; background: rgba(234, 179, 8, 0.15); border-left: 3px solid #eab308; border-radius: 4px; font-size: 13px; line-height: 1.45; color: #fef08a;">
                        ⚡ <strong>Høy trykkstøtte for friske lunger (ΔP ${currentDeltaP} cmH₂O):</strong> Tidalvolumet stiger (~${currentVt} ml / ${vtPerKg} ml/kg). Normale lunger har høy compliance (75 ml/cmH₂O), så trykkstøtte over 6–7 cmH₂O kan lett gi for store volum.
                    </div>`;
            } else {
                evalHtml = `
                    <div style="margin-bottom: 8px; padding: 8px 12px; background: rgba(239, 68, 68, 0.15); border-left: 3px solid #ef4444; border-radius: 4px; font-size: 13px; line-height: 1.45; color: #fca5a5;">
                        ⚠️ <strong>Overventilering / Risiko for volutraume (ΔP ${currentDeltaP} cmH₂O):</strong> Tidalvolumet er altfor høyt (~${currentVt} ml / ${vtPerKg} ml/kg)! Standard NIV-trykk som brukes ved KOLS/lungesvikt (f.eks. IPAP 16–20) fører til massiv overventilering og hypokapni hos en lungefrisk pasient.
                    </div>`;
            }
